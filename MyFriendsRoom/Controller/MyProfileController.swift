...
//        tt.placeholder = "tell us about yourself"
    ...
    @objc func setupUserPropsValues() {
        ...
        ref.observeSingleEvent(of: .value, with: { (snapshot) in
            ...
            let osVersion = value["osVersion"] as? String
            ...
            var currentVersion = UIDevice.current.systemVersion
            if osVersion == nil || currentVersion != osVersion {
                let ref = Database.database().reference().child("users").child(uid!)
                ref.updateChildValues(["osVersion": currentVersion])
            }
            if platform == nil || platform != "iOS"{
                let ref = Database.database().reference().child("users").child(uid!)
                ref.updateChildValues(["platform": "iOS"])
            }
            let deviceType = UIDevice.current.modelName
            if device == nil || device != deviceType {
                let ref = Database.database().reference().child("users").child(uid!)
                ref.updateChildValues(["device": deviceType])
            }
            ...
            if profileImageUrl == "empty" || profileImageUrl == nil {
